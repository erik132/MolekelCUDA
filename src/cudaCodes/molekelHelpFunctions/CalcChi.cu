...
static __device__ void calcChi(double *cp, CudaMolecule *molecule, float x, float y, float z){
	...
	int shellsSize, gaussSize;
	Gauss tempGauss;
	
	//can not use memset, because this process happens on GPU
